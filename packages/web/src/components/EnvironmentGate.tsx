import { GoogleAuthProvider, signInWithPopup, signOut } from 'firebase/auth';
import { ReactNode, useCallback, useMemo, useState } from 'react';
import { gateAuth } from '../firebase';

interface EnvironmentGateProps {
  children: ReactNode;
  environment: string;
}

// This gate's own "authorized" state is tracked in sessionStorage, deliberately
// separate from any Firebase Auth `user` -- the gate signs in through its own
// isolated Firebase App instance (`gateAuth`, see firebase.ts) so passing the
// gate never signs the visitor into anything the rest of the site observes.
const GATE_SESSION_KEY = 'ms_env_gate_authorized';
const GATE_EMAIL_KEY = 'ms_env_gate_email';

/**
 * OAuth-based environment gate using Firebase Auth + Google Sign-In.
 *
 * Configuration via build-time environment variables:
 * - VITE_ALLOWED_EMAIL_DOMAINS: comma-separated domains (e.g. "company.com,trusted.org")
 * - VITE_ALLOWED_EMAILS: comma-separated exact emails (e.g. "alice@example.com,bob@example.com")
 *
 * Access is granted if the signed-in email matches an allowed domain OR an
 * entry in the allowed-emails list. If no allowlist is configured, access is
 * denied (fails closed).
 *
 * Only the publicly reachable non-prod web deployments
 * (modulo-squares-{dev,staging}.web.app) are gated. Production and localhost
 * render children unconditionally.
 */
export default function EnvironmentGate({
  children,
  environment,
}: EnvironmentGateProps) {
  const [isAuthorized, setIsAuthorized] = useState(() => {
    try {
      return sessionStorage.getItem(GATE_SESSION_KEY) === '1';
    } catch {
      return false;
    }
  });
  const [authorizedEmail, setAuthorizedEmail] = useState(() => {
    try {
      return sessionStorage.getItem(GATE_EMAIL_KEY) || '';
    } catch {
      return '';
    }
  });
  const [error, setError] = useState('');
  const [isSigningIn, setIsSigningIn] = useState(false);

  // A local dev server is not a public attack surface, so skip the gate on
  // localhost -- otherwise `vite dev` would be unusable without OAuth config.
  const isLocalhost =
    typeof window !== 'undefined' &&
    /^(localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\])$/.test(
      window.location.hostname
    );

  const requiresAuth =
    !isLocalhost &&
    (environment === 'staging' || environment === 'development');

  const allowedConfig = useMemo(() => {
    const domains = (import.meta.env.VITE_ALLOWED_EMAIL_DOMAINS || '')
      .split(',')
      .map((d) => d.trim().toLowerCase())
      .filter(Boolean);

    const emails = (import.meta.env.VITE_ALLOWED_EMAILS || '')
      .split(',')
      .map((e) => e.trim().toLowerCase())
      .filter(Boolean);

    return { domains, emails };
  }, []);

  const isEmailAuthorized = useCallback(
    (userEmail: string | null | undefined): boolean => {
      if (!userEmail) return false;

      const { domains, emails } = allowedConfig;

      // No allowlist configured -> deny (fail closed).
      if (domains.length === 0 && emails.length === 0) {
        return false;
      }

      if (emails.includes(userEmail.toLowerCase())) {
        return true;
      }

      const [, domain] = userEmail.split('@');
      if (domain && domains.includes(domain.toLowerCase())) {
        return true;
      }

      return false;
    },
    [allowedConfig]
  );

  const handleSignIn = async () => {
    setError('');
    setIsSigningIn(true);

    try {
      const provider = new GoogleAuthProvider();
      provider.setCustomParameters({ prompt: 'select_account' });
      const credential = await signInWithPopup(gateAuth, provider);
      const signedInEmail = credential.user.email;

      if (isEmailAuthorized(signedInEmail)) {
        try {
          sessionStorage.setItem(GATE_SESSION_KEY, '1');
          sessionStorage.setItem(GATE_EMAIL_KEY, signedInEmail || '');
        } catch {
          // sessionStorage unavailable (private mode, etc.) -- gate still works
          // for this render, it just won't survive a reload.
        }
        setAuthorizedEmail(signedInEmail || '');
        setIsAuthorized(true);
        setError('');
      } else {
        setError(
          `Your email (${signedInEmail}) is not authorized to access this environment.`
        );
      }

      // gateAuth isn't observed by anything else -- this is just hygiene.
      await signOut(gateAuth).catch(() => {});
    } catch (err) {
      const authError = err as { code?: string; message?: string };
      if (authError.code !== 'auth/popup-closed-by-user') {
        setError(authError.message || 'Failed to sign in. Please try again.');
      }
    } finally {
      setIsSigningIn(false);
    }
  };

  const handleSignOut = () => {
    try {
      sessionStorage.removeItem(GATE_SESSION_KEY);
      sessionStorage.removeItem(GATE_EMAIL_KEY);
    } catch {
      // ignore
    }
    setAuthorizedEmail('');
    setIsAuthorized(false);
  };

  if (!requiresAuth) {
    return <>{children}</>;
  }

  if (isAuthorized) {
    return (
      <div>
        {children}
        <div className="fixed bottom-4 right-4 z-50">
          <button
            onClick={handleSignOut}
            className="rounded bg-gray-800 px-3 py-2 text-xs text-white shadow-lg transition-colors hover:bg-gray-900"
            title={`Team access granted to ${authorizedEmail}`}
          >
            {authorizedEmail} (Revoke access)
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="flex min-h-dvh items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-lg">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100">
            <svg
              className="h-8 w-8 text-gray-600"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"
              />
            </svg>
          </div>
          <h1 className="mb-2 text-2xl font-bold text-gray-900">
            {environment.charAt(0).toUpperCase() + environment.slice(1)}{' '}
            Environment
          </h1>
          <p className="mb-4 text-gray-600">
            This is a restricted {environment} environment. Access is controlled
            via team authorization.
          </p>

          {error && (
            <div className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
              {error}
            </div>
          )}
        </div>

        <button
          onClick={handleSignIn}
          disabled={isSigningIn}
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-gray-300 bg-white px-6 py-3 font-medium text-gray-900 transition-colors hover:bg-gray-50 disabled:opacity-50"
        >
          <svg className="h-5 w-5" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          {isSigningIn ? 'Signing in...' : 'Sign in with Google'}
        </button>

        <div className="mt-6 text-center">
          <p className="text-xs text-gray-500">
            Access is managed via team authorization. Contact your team lead if
            you don't have access.
          </p>
        </div>
      </div>
    </div>
  );
}
